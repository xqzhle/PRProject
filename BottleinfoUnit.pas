...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, Buttons, Grids, BaseGrid, AdvGrid, IniFiles, ComCtrls, DateUtils, ADODB, DB, ExtCtrls, OleServer, AcReport_TLB;

type
  TBottleInfoForm = class(TForm)
    BitBtn1: TBitBtn;
    BitBtn2: TBitBtn;
...
var
  BottleInfoForm: TBottleInfoForm;

implementation

uses
   DbUnit,zcomm,MainUnit, BottleListUnit;
...
procedure TBottleInfoForm.BitBtn1Click(Sender: TObject);
begin
//  if edit2.text = '' then
//  begin
//    showmessage('请读取钢瓶标签');
...
//  end;
  ...
  //钢瓶规格
  if ( (trim(edit3.Text) <> '') and (Caption = '钢瓶信息-新增')) then
  begin
    try
...
//        end;
  ...
  else if (trim(edit3.Text) <> '') and (Caption = '钢瓶信息-修改') then
  begin
    try
      if trim(edit2.Text) <>'' then
      begin
...
procedure TBottleInfoForm.BitBtn2Click(Sender: TObject);
begin
  ModalResult := mrCancel;
  Close;
end;

procedure TBottleInfoForm.BitBtn3Click(Sender: TObject);
begin
  if  (trim(edit18.Text) <> '') and (Caption = '钢瓶信息-修改') then
  begin
...
procedure TBottleInfoForm.Button1Click(Sender: TObject);
var
  s1, s2, s3: string;
  jbar: string;
begin
...
procedure TBottleInfoForm.FormShow(Sender: TObject);
var
  s1, s2: string;
//  leng: Integer;
//  myinifile: Tinifile;
...
//  myinifile.Free;
  ...
  begin
    ...
    if  (Caption = '钢瓶信息-修改') then
    begin

     //  try
      Edit14.Text := Loginname;
      Edit16.Text := gettime;
      s1 := Data1.sp.FieldByName('状态').AsString;
      s2 := Data1.sp.FieldByName('产权').AsString;
      if s1 = '在用' then
      ...
      with Data1 do
      begin
        edit2.Text := sp.FieldByName('钢瓶标签').AsString;
        edit3.Text := sp.FieldByName('钢瓶编号').AsString;
        edit7.Text := sp.FieldByName('备注').AsString;
        edit9.Text := sp.FieldByName('钢瓶重量').AsString;
        edit8.Text := sp.FieldByName('钢瓶条码').AsString;
        edit10.Text := sp.FieldByName('检验编号').AsString;
        edit18.Text := sp.FieldByName('序号').AsString;
        combobox6.Text := sp.FieldByName('检测单位').AsString;
        combobox4.Text := sp.FieldByName('钢瓶类型').AsString;
        ComboBox7.ItemIndex := sp.FieldByName('检验次数').AsInteger - 1;
        datetimepicker1.DateTime := sp.FieldByName('制造日期').Value;
        datetimepicker2.DateTime := sp.FieldByName('最新检验日期').Value;
        datetimepicker3.DateTime := sp.FieldByName('下检日期').Value;
        datetimepicker4.DateTime := sp.FieldByName('报废日期').Value;
      end;
...
procedure TBottleInfoForm.ComboBox7Change(Sender: TObject);
begin
...
procedure TBottleInfoForm.DateTimePicker1Change(Sender: TObject);
begin
  if DateTimePicker1.Date < StrToDate('2006-07-01') then
    datetimepicker4.date := datetimepicker1.Date + 365 * 15 + 3
...
procedure TBottleInfoForm.DateTimePicker2Change(Sender: TObject);
var
 s:string;
begin
  datetimepicker3.date := datetimepicker2.Date + 365 * 4 + 1;
...
procedure TBottleInfoForm.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  if Not (key in ['0'..'9', #8,'a'..'z' ,'A'..'Z']) then  key:=#0;
end;

procedure TBottleInfoForm.Edit9KeyPress(Sender: TObject; var Key: Char);
begin
// if not (Key in ['0'..'9','.',#8,#13]) then   Key:=#0;
end;

procedure TBottleInfoForm.RadioButton10Click(Sender: TObject);
begin
  datetimepicker4.date := datetimepicker1.Date + 365 * 12 + 4;
end;

procedure TBottleInfoForm.RadioButton1Click(Sender: TObject);
begin//钢瓶规格
//bo_statu:=(sender as TRadioButton).TabOrder;
end;

procedure TBottleInfoForm.RadioButton5Click(Sender: TObject);
begin//钢瓶产权
//bo_right:=(sender as TRadioButton).TabOrder;
end;

procedure TBottleInfoForm.RadioButton9Click(Sender: TObject);
begin
  datetimepicker4.date := datetimepicker1.Date + 365 * 8 + 3;
end;

procedure TBottleInfoForm.ComboBox1Change(Sender: TObject);
begin
