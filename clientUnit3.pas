...
type
  TForm1 = class(TdxRibbonForm)// IHTTPChatServerEvents
...
uses clientUnit1, data, wpctiform, Unit28, Unit29, Unit30, Unit31,PRRead_TLBUnit,
  Unit32, Unit33, Unit41, Unit88, Unit89, datecomm, Unit71,ShellAPI,Unit49,
  SelajUnit,EncdDecd,WxUnit,CmoneyUnit,CVRDLL,zcomm,SqgUnitpas,Unit72,Unit96;
...
function TForm1.selectdial(const dialn: string):Boolean;
begin
 try
  Data1.sqlcmd1.Close;
...
procedure  TForm1.addbill(const typebill:string);
begin
  if Edit1.Text<>'' then
  begin
   try
     Data1.sds1.Close;
     Data1.sds1.SQL.Text:=' insert into ttbill (ext,bill,linkid,type,billdate) values ('''+ext+''','''+edit1.Text+''','''+usname+''','''+typebill+''','''+datetimetostr(sdatetime)+''')' ;
     Data1.sds1.ExecSQL;
    except
 ...
 if DBEdit1.Text='' then
      ...
      begin
       ...
       try
         ...
         Data1.sds1.Close;
         Data1.sds1.SQL.Text:='insert into tttousu (shopid,memo,custid,creid,credate,shopname,custname,address,tel) values ('''+edit2.Text+''','''+memo2.Text+''','''+dbedit1.Text+''','''+djsn+''','''+datetimetostr(sdatetime)+''','''+dbcombobox2.Text+''','''+dbedit2.Text+''','''+dbedit6.Text+''','''+dbedit3.Text+''')';
         Data1.sds1.ExecSQL;
         Data1.sds1.Close;
...
procedure TForm1.Ad(Sender: TObject);
var
  selectstr:string;
begin
   if Edit5.Text='' then Exit
   else
   begin                        //a.Grade as Grade, ,a.fax as fax
      selectstr:='select top 20 a.id as id,a.fmoney as fmoney,a.namec as namec,a.tel as tel,a.Address as Address,a.contact as contact,a.ywy as ywy,a.SendPeriod as SendPeriod,a.Remark as Remark,a.fjf as fjf,'+
      ' a.CreateDate as CreateDate,a.psid as psid,a.TypeC as TypeC,b.shopname as shopname,a.qyid as qyid,a.jdid as jdid,d.namec as qyname,e.namec as jdname,a.sqg as sqg,a.hynumber as hynumber, '+
      ' c.Type_Name as typename,a.BarCode as barcode,a.bayday as bayday,a.ajdate as ajdate,a.kfsn as kfsn,a.gpstype as gpstype from tbCustomer_Info a left join tshop b on (a.psid=b.shopid) '+
...
begin            //咨询
    if DBEdit1.Text='' then
      ...
      try
        ...
        Data1.sds1.Close;
        Data1.sds1.SQL.Text:='insert into ttzixun (shopid,memo,custid,creid,credate,shopname,custname,address,tel) values ('''+edit2.Text+''','''+memo3.Text+''','''+dbedit1.Text+''','''+usname+''','''+datetimetostr(sdatetime)+''','''+dbcombobox2.Text+''','''+dbedit2.Text+''','''+dbedit6.Text+''','''+dbedit3.Text+''')';
        Data1.sds1.ExecSQL;
        Data1.sds1.Close;
...
begin         //发布信息
      ...
      begin
        ...
        str:= 'insert into ttxinxi (shopid,memo,creid,credate,shopname,stype) values ('''+edit2.Text+''','''+zh+''','''+usname+''','''+datetimetostr(sdatetime)+''','''+dbcombobox2.Text+''',''0'')';
       end;
       if AdvOfficeRadioButton4.Checked=True then
      begin

        str:= 'insert into ttxinxi (shopid,memo,creid,credate,shopname,ins,stype) values ('''+edit10.Text+''','''+memo4.Text+''','''+usname+''','''+datetimetostr(sdatetime)+''','''+combobox5.Text+''',''0'',''1'')';
       end;
       try
...
procedure TForm1.AdvGlowButton16Click(Sender: TObject);
begin
...
procedure TForm1.AdvGlowButton18Click(Sender: TObject);
//var
...
procedure TForm1.AdvGlowButton19Click(Sender: TObject);
var
 List: TStringList;
 i:Integer;
 zh:string;
...
procedure TForm1.AdvGlowButton20Click(Sender: TObject);
var
 rpt_path,djsn:string;
begin
       rpt_path := ExtractFilePath(ParamStr(0)) + '\report\';
      ...
      begin
        ...
        ACRptEngine1.AddVariable('counttel', dbedit3.Text) ;
        ACRptEngine1.AddVariable('shopname', dbcombobox2.Text) ;
        ACRptEngine1.AddVariable('bdate', datetostr(sdatetime)) ;
...
procedure TForm1.AdvGlowButton24Click(Sender: TObject);
var
  iRet :integer;   //iPort
...
procedure TForm1.AdvGlowButton26Click(Sender: TObject);
begin
  if DBEdit1.Text='' then Exit;
      ...
      '  from tb_khgp a left join tshop b on (a.shopid=b.shopid) where a.khbh='''+Form1.DBEdit1.Text+''' order by a.cdate desc';
      Data1.ds12.Open;
    ...
    try
      ...
      begin
        Data1.ClientDataSet6.SQL.Text:='update tbCustomer_Info set barcode='''+Trim(DBEdit1.Text)+''',psid='''+edit2.Text+''',typec='''+edit3.Text+''',NameC='''+dbedit2.Text+''',tel='''+dbedit3.Text+''',Contact='''+dbedit4.Text+''', '+
         'Address='''+dbedit6.Text+''',SendPeriod='''+dbedit7.Text+''',Remark='''+dbmemo1.Text+''',hynumber='''+dbedit8.Text+''',yj='''+yj+''',qyid='''+cbb3.Text+''',jdid='''+cbb4.Text+''','+
         ' bayday='''+dbedit10.Text+''',kfsn='''+dbedit11.Text+''',gpstype='''+combobox6.Text+''',sqg='''+combobox9.Text+''',ywy='''+combobox8.Text+''',UpdateDate='''+datetimetostr(sdatetime)+''',UpdateUser='''+usname+''' where id='''+dbedit13.text+'''';
      end
      else
      begin
        ...
        Data1.ClientDataSet6.SQL.Text:='update tbCustomer_Info set barcode='''+Trim(DBEdit1.Text)+''',psid='''+edit2.Text+''',typec='''+edit3.Text+''',NameC='''+dbedit2.Text+''',tel='''+dbedit3.Text+''',Contact='''+dbedit4.Text+''', '+
         'Address='''+dbedit6.Text+''',SendPeriod='''+dbedit7.Text+''',Remark='''+dbmemo1.Text+''',hynumber='''+dbedit8.Text+''',yj='''+yj+''',qyid='''+cbb3.Text+''',jdid='''+cbb4.Text+''','+
         ' bayday='''+dbedit10.Text+''',kfsn='''+dbedit11.Text+''',sqg='''+combobox9.Text+''',ywy='''+combobox8.Text+''',UpdateDate='''+datetimetostr(sdatetime)+''',UpdateUser='''+usname+''',gpstype='''+combobox6.Text+''' '+imgstr+' where id='''+dbedit13.text+'''';
      end;
      sfshow := True;
...
procedure TForm1.sfbutt(Sender: TObject);
begin
    try
...
procedure TForm1.AdvGlowButton48Click(Sender: TObject);
var
mm:Integer;
begin
      Label18.Caption:='';
     ...
     if DBEdit1.Text='' then Exit;
      ...
      ' tbisent a left join tbisentprice b on a.bisentid=b.bisentid where (a.corpid='''+Form1.DBEdit1.Text+''')   and (a.type<>3)'+
...
//      ' credate as 收费日期,crename as 收费人  from tbskbottle where barcode='''+Form1.DBEdit1.Text+''' ';
      ...
      ' crename as 收款人 from tbskbottle where barcode='''+Form1.DBEdit1.Text+''' and type=0 and (stype=0 or stype=1 or stype=2) order by id desc';
      Data1.ds12.Open;
      if Data1.ds12.IsEmpty then  Data1.ds12.Close;
end;
...
var
shopid1,shopname,yudate,typeid,status,typename,empname,jz,optdate,sl:string;//门店id，门店名，预约时间，订单类型
jf,smon:Real;
i:Integer;
begin
  if addyes then
...
//     end;
     ...
     else  optdate:=datetimetostr(sdatetime);
     try
       if not Data1.ds11.IsEmpty then
       begin
         if Data1.ds11.RecordCount>10 then
         begin
            ShowMessage('数量大于10,请检查！');
            Exit;
         end;
         Data1.ds11.First;
         for I := 0 to Data1.ds11.RecordCount-1 do
         begin
           if DBEdit8.Text <>'' then
           begin
             Data1.sds111.Close;
             Data1.sds111.SQL.Text:='select onejf from tpriceinfo where name='''+Data1.ds11.FieldByName('商品名称').AsString+'''';
             Data1.sds111.Open;
             if not Data1.sds111.IsEmpty then
             begin
               if Data1.sds111.FieldByName('onejf').AsString<>'0' then
               begin
                  jf:=strtofloat(Data1.sds111.FieldByName('onejf').AsString)* strtofloat(Data1.ds11.FieldByName('数量').AsString);
               end;
             end;
           end;
           dj:= Data1.ds11.FieldByName('单价').AsString;
           sqf:=Data1.ds11.FieldByName('送气费').AsString;
           yh:=Data1.ds11.FieldByName('优惠').AsString;
           je:=Data1.ds11.FieldByName('金额').AsString;
           sl:=Data1.ds11.FieldByName('数量').AsString;
           smon:=smon+strtofloat(Data1.ds11.FieldByName('金额').AsString);
//           try
             Data1.sds1.Close;                             //,typename='''+typename+''' 钢瓶类型       '''+optdate+'''
             Data1.sds1.SQL.Text:='update tbisent set empname='''+empname+''',status='''+status+''',optdate=GETDATE(),optid='''+usname+''',type='''+typeid+''','+
             ...
             if AdvOfficeCheckBox2.Checked then
             begin
              ShowSmoothMessage('是自带瓶充装客户,现在马上打印充装票！',tsOffice2007Luna);
              b1:=DBEdit1.Text;    //客户条码
              b2:=Data1.ds11.FieldByName('商品名称').AsString;  //商品类型
              b3:=edit11.Text;  //数量
              b4:=usName;          //收款员
              b5:=dj;              //单价
              b6:=FormatFloat('0.00',strtofloat(dj)*strtofloat(b3));//合计
              b7:=bottleid; //客户瓶号
              b9:= Data1.ds11.FieldByName('单号').AsString;//订单号
...
//           end;
         ...
         ShowSmoothMessage('订单保存成功！',tsOffice2007Luna);
...
//         addbill('订气');
       ...
       ShowSmoothMessage('订单保存失败！',tsOffice2007Luna);
     end;

  end
  else   ShowSmoothMessage('没有添加商品不能保存订单！',tsOffice2007Luna);
end;
...
//            ShowMessage(Data1.ds11.FieldByName('单号').AsString);
        ...
        ShowSmoothMessage('订单取消成功！',tsOffice2007Luna);
        AdvGlowButton3.Enabled:=True;
        addyes:=False;
        Data1.ds11.Close;
        Data1.ds11.Open;
   ...
   else  ShowSmoothMessage('没有可取消的订单！',tsOffice2007Luna);
end;
...
begin             //报修
      ...
      try
        ...
        ' values ('''+edit2.Text+''','''+memo1.Text+''','''+dbedit1.Text+''','''+djsn+''','''+datetimetostr(sdatetime)+''','''+dbcombobox2.Text+''','''+dbedit2.Text+''','''+dbedit6.Text+''','''+dbedit3.Text+''')';
        Data1.sds1.ExecSQL;
        Data1.sds1.Close;
...
procedure TForm1.AdvOfficePage2Show(Sender: TObject);
begin
...
procedure TForm1.AdvOfficePage4Hide(Sender: TObject);
begin
  Data1.ds12.close;
end;
...
procedure TForm1.AdvOfficePage5Show(Sender: TObject);
//var
// ss:TStringStream;
// ms:TMemoryStream;
// jpg:TJpegImage;
...
function TForm1.StringToBitmap(imgStr:string):TBitmap;
var ss:TStringStream;
    ms:TMemoryStream;
    bitmap:TBitmap;
begin
...
procedure TForm1.AdvOfficePager22Hide(Sender: TObject);
begin
   Data1.ds12.close;
end;

procedure TForm1.AdvOfficeRadioButton3Click(Sender: TObject);
begin
      Memo4.Text:='时间：'+TimeToStr(Time)+' 此客户催气，请尽快送气！';
      Edit10.Visible:=false;
      ComboBox5.Visible:=False;
...
procedure TForm1.AdvOfficeRadioButton4Click(Sender: TObject);
begin
      Memo4.Lines.Clear;
      Edit10.Visible:=True;
...
procedure TForm1.AdvSmoothButton10Click(Sender: TObject);
begin
  wxstype:=1;
...
procedure TForm1.AdvSmoothButton11Click(Sender: TObject);
begin
  wxstype:=2;
...
procedure TForm1.AdvSmoothButton13Click(Sender: TObject);
begin
...
procedure TForm1.AdvSmoothButton16Click(Sender: TObject);
begin
      Form88.ShowModal;
end;

procedure TForm1.AdvSmoothButton17Click(Sender: TObject);
begin
    Form89.ShowModal;
end;
...
procedure TForm1.AdvSmoothButton3Click(Sender: TObject);
begin                                //呼叫转移
...
procedure TForm1.AdvSmoothButton4Click(Sender: TObject);
begin
       ctisrv2.cmdOffHook(extid);
      timer5.Enabled:=True;
...
procedure TForm1.AdvSmoothButton5Click(Sender: TObject);
begin
    ctisrv2.cmdOffHook(extid);
    timer7.Enabled:=True;
...
procedure TForm1.AdvSmoothButton6Click(Sender: TObject);
begin
    CTISrv2.cmdOnHook(extid);  //挂机
   // shows
end;

procedure TForm1.AdvSmoothButton7Click(Sender: TObject);
 var
 dats:string;
begin
    dats:=DateToStr(Date)+' 00:00:00';
    Data1.cmd2.Close;
    Data1.cmd2.SQL.Text:='select billdate as 接听时间,bill as 来电号码,linkid as 接线员,type as 性质 from ttbill where  (billdate>='''+dats+''') and (billdate<='''+DateTimeToStr(sdatetime)+''') and (linkid='''+usname+''') order by billdate desc ';
    Data1.cmd2.Open;
...
procedure TForm1.AdvSmoothButton8Click(Sender: TObject);
begin
...
procedure TForm1.CTISrv2calledRing(ASender: TObject; ID, UserType, assID,
  assUserType: SmallInt; const assPhone, assUserName: WideString; mixdata,
...
procedure TForm1.CTISrv2connected(ASender: TObject; ID, UserType, IOFlag, assID,
  assUserType: SmallInt; const assPhone, assUserName: WideString; mixdata,
...
procedure TForm1.CTISrv2dial(ASender: TObject; ID, UserType, assID,
  assUserType: SmallInt; const assPhone, assUserName: WideString; mixdata,
...
procedure TForm1.CTISrv2idle(ASender: TObject; ID, UserType, assID,
  assUserType: SmallInt; const assPhone, assUserName: WideString; mixdata,
...
procedure TForm1.CTISrv2loginErr(ASender: TObject; ID: SmallInt; const errCode,
  errDesc: WideString);
begin
...
procedure TForm1.CTISrv2loginOK(ASender: TObject; ID: SmallInt);
begin
...
procedure TForm1.CTISrv2otherIDTisInfoArrival(ASender: TObject; ID, UserType,
  dstID, dstUserType, dstStatu: SmallInt; srvCls: Byte; const dstPhone,
  dstUserName: WideString; mixdata: SmallInt);
begin
...
procedure TForm1.CTISrv2phoneDisplay(ASender: TObject; ID, UserType, assID,
  assUserType: SmallInt; const assPhone, assUserName: WideString;
...
begin
  ...
  callNOAsi:=False;
  sfshow := true;
  selectstr:='select a.id as id,a.fmoney as fmoney,a.namec as namec,a.tel as tel,a.Address as Address,a.fax as fax,a.contact as contact,a.ywy as ywy,a.SendPeriod as SendPeriod,a.Remark as Remark,a.fjf as fjf,'+
      ' a.CreateDate as CreateDate,a.psid as psid,a.Grade as Grade,a.TypeC as TypeC,b.shopname as shopname,a.qyid as qyid,a.jdid as jdid,d.namec as qyname,e.namec as jdname,a.sqg as sqg,a.hynumber as hynumber, '+
      ' c.Type_Name as typename,a.BarCode as barcode,a.bayday as bayday,a.ajdate as ajdate,a.kfsn as kfsn,a.gpstype as gpstype from tbCustomer_Info a left join tshop b on (a.psid=b.shopid) '+
      ' left join tbCustomer_Type c on (a.TypeC=c.Type_ID) left join tbkh_qy d on (a.qyid=d.id) left join tbkh_jd e on (a.jdid=e.id)';
     str:=assPhone;
     if str<>'' then
     ...
     Delete(str,1,1);
     Edit1.Text:=str;
     CnTrayIcon1.BalloonHint('有新来电',str,btInfo,20 );                              //or fax='''+str+'''
     Data1.ClientDataSet3.Close;
...
procedure TForm1.CTISrv2UDPDataArrival(ASender: TObject; ID, UserType, status,
  IOFlag, mixdata, assID, assUserType: SmallInt; const assPhone,
  assUserName: WideString);
begin
...
procedure TForm1.DBAdvGrid1Resize(Sender: TObject);
begin
      DBAdvGrid1.AutoSizeColumns(False,2); //根据内容调节列宽
end;

procedure TForm1.DBAdvGrid2Resize(Sender: TObject);
begin
     DBAdvGrid2.AutoSizeColumns(False,2); //根据内容调节列宽
end;

procedure TForm1.DBAdvGrid3DblClick(Sender: TObject);
begin
...
procedure TForm1.DBAdvGrid3GetCellColor(Sender: TObject; ARow, ACol: Integer;
  AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
var
  date1:TDateTime;
begin
...
procedure TForm1.DBAdvGrid3Resize(Sender: TObject);
begin
  DBAdvGrid3.AutoSizeColumns(False,6); //根据内容调节列宽
end;

procedure TForm1.DBAdvGrid4Resize(Sender: TObject);
begin
    DBAdvGrid4.AutoSizeColumns(False,2);
end;

procedure TForm1.DBAdvGrid5Click(Sender: TObject);
begin
...
procedure TForm1.DBAdvGrid5SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
begin
  if (ACol=3) then
    begin
...
procedure TForm1.DBComboBox11Click(Sender: TObject);
begin
try
       Data1.ClientDataSet1.Close;
...
procedure TForm1.DBComboBox23Click(Sender: TObject);
begin
     try
       Data1.ClientDataSet1.Close;
...
procedure TForm1.DBComboBox2Click(Sender: TObject);
begin
  try
    if DBEdit1.Text='' then  Exit;
    Data1.ClientDataSet1.Close;
    ...
    begin
      ...
      Data1.ClientDataSet1.Close;
      Data1.ClientDataSet1.SQL.Text:='select EmpName from tAchEmployee where shopid='''+Edit2.Text+''' ';
      Data1.ClientDataSet1.Open;
      if not Data1.ClientDataSet1.IsEmpty then
      begin
        while not Data1.ClientDataSet1.Eof do
...
begin
     ...
     DateTimePicker1.Date:=sdatetime;
     DateTimePicker3.Date:=DateTimePicker1.Date;
     DateTimePicker2.Time:=sdatetime;
     ...
     times:=5;
       ...
       DBAdvGrid3.SetStyle(gsOffice2007Luna);
       if ext<>'' then
       begin
         Data1.ClientDataSet1.Close;
       ...
       Data1.ClientDataSet1.Close;
       Data1.ClientDataSet1.SQL.Text:='select ShopName,shopid from tShop where shopid<>0';
       Data1.ClientDataSet1.Open;
...
//       ComboBox4.Items.Clear;
//       Data1.ClientDataSet1.Close;
//       Data1.ClientDataSet1.SQL.Text:='select shopName from tshop where shopid<>0';
//       Data1.ClientDataSet1.Open;
//       while not Data1.ClientDataSet1.eof do
//       begin
//         ComboBox4.Items.Add(Data1.ClientDataSet1.Fields[0].AsString);
...
//       Data1.ClientDataSet1.Close;
  ...
  try
    ...
    begin
      CTISrv2.loginType:=6;
      CTISrv2.remoteHostIP:=pbxip;
      CTISrv2.UDPRemoteHost:=pbxip;
      CTISrv2.login(ext,'');
    end;
    Edit13.Clear;
...
begin  //让panel 居中 不管分辨率多少
    AdvSmoothPanel2.Left:=round((form1.Width-AdvSmoothPanel2.Width)/2);
    AdvSmoothPanel2.top:=round((form1.Height-AdvSmoothPanel2.Height)/2);
...
procedure TForm1.RingOutTimerTimer(Sender: TObject);
begin
   RingOutTimer.Enabled:=False;
   addbill('未接');
end;
